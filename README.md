...
| [`app/resources/tracks/`](app/resources/tracks/) | Os cursos que o app oferece ao aluno. **Hoje no disco existe um:** `python-iniciante` (7 módulos, 112 aulas — ver `docs/18-estado-da-fabricacao-dos-cursos.md`); os outros três (`python-intermediario`, `python-avancado`, `python-especialista`) estão planejados — o desenho integral está no [`docs/17-trilha-python.md`](docs/17-trilha-python.md). |
