...
| [`docs/20-trilha-rust.md`](docs/20-trilha-rust.md) | O mesmo para os quatro cursos de Rust: a cadeia com o `rust-iniciante` desenhado aula a aula (8 módulos, 101 aulas) sobre o vocabulário fechado de 191 chaves do adaptador — e **autorado**: o iniciante está no disco (o estado, os números e as dívidas de engine com prova estão em `docs/18` §8 e no próprio docs/20 §⚑). |
...
- **nenhum código do app roda na CI.** O `gate.yml` roda os 5 gates bash e o `install.sh`, e os 5
  gates têm zero ocorrências de `app/`. Consequência medida (2026-09-17): a suíte do app fecha
