...
| [`app/resources/tracks/`](app/resources/tracks/) | Os cursos que o app oferece ao aluno. **Hoje no disco existe um:** `python-iniciante` (7 módulos, 112 aulas — ver `docs/18-estado-da-fabricacao-dos-cursos.md`); os outros três (`python-intermediario`, `python-avancado`, `python-especialista`) estão planejados — o desenho integral está no [`docs/17-trilha-python.md`](docs/17-trilha-python.md). **A cadeia de Rust** (`rust-iniciante` → `rust-intermediario` → `rust-avancado` → `rust-especialista`) já tem contrato de conteúdo — [`docs/20-trilha-rust.md`](docs/20-trilha-rust.md) — e o adaptador da linguagem está na engine (`app/electron/main/engine/lang/rust.ts`); nenhuma trilha Rust está autorada ainda. |
...
| [`docs/20-trilha-rust.md`](docs/20-trilha-rust.md) | O mesmo para os quatro cursos de Rust: a cadeia com o `rust-iniciante` desenhado aula a aula (8 módulos, 101 aulas) sobre o vocabulário fechado de 190 chaves do adaptador. |
