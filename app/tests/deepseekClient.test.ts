 ...
 * 5xx com corpo, falha de rede, sem chave (KEY_MISSING), timeout via AbortController,
 * headers corretos e model default literal.
 */
...
test('chatCompletion: 200 → parsea content/model/usage e envia headers + model default', async () => {
  ...
  assert.equal(res.model, 'deepseek-v4-flash-0731');
  ...
  assert.equal(body.model, 'deepseek-v4-flash-0731'); // literal do contrato
...
test('chatCompletion: resposta sem choices content → NETWORK', async () => {
