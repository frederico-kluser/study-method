 ...
 * `resolveFeedbackProvider` ('local' | 'openrouter') e devolve a CHAVE de
 * tradução que o componente usa com `t()`. `null` → sem chip (nenhum provedor
 * decidido).
 *
...
export type ProviderChipKey = 'challenge.providerLocal' | 'challenge.providerOpenrouter';
...
export function feedbackProviderChipKey(provider: FeedbackProvider | null): ProviderChipKey | null {
  ...
  if (provider === 'openrouter') return 'challenge.providerOpenrouter';
  return null;
}